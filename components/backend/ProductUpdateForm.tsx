...
const ProductUpdateForm = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    ...
    if(tags?.length){
      setTag(data?.tags?.length ?data?.tags?.map((item)=>({label:item.name,value:item.id})):[]);
    }
    if(data?.userId){

      setFarmer(data.userId);
    }
  }, [data,reset,tags.length]);
  return (
    <div className="">
      <FormContainer>
          ...
          >
            ...
            <div className="w-full lg:col-span-2">
              ...
              <ToggleInput
                ...
                name="isActive"
              />
              <SubmitButton loading={isLoading} title="Update Product" />
            </div>
          </form>
        </div>
      </FormContainer>
